...
describe('First unit test', function() {
    ...
    it('Listener unit test for GraphModel', function() {
        ...
        expect(firstListener.calledWith('GRAPH_SELECTED_EVENT'), 'GraphModel argument verification').to.be.true;
    ...
    it('Listener unit test for ActivityStoreModel', function() {
        ...
        expect(firstListener.calledWith('ACTIVITY_DATA_ADDED_EVENT'), 'ActivityStoreModel argument verification').to.be.true;

        var secondListener = sinon.spy();
        activityStoreModel.addListener(secondListener);
        activityStoreModel.removeActivityDataPoint(new ActivityData());
    ...
    it('Check if two activities with the same type are grouped together by adding the time spent on each of them for table analysis', function(){
        var activityStoreModel = new ActivityStoreModel();
        var  dic;
        activityStoreModel.addActivityDataPoint(new ActivityData("A",dic,1));
        activityStoreModel.addActivityDataPoint(new ActivityData("A",dic,2));

        expect(activityStoreModel.activityForDisplay.length).to.equal(1);
        expect(activityStoreModel.activityForDisplay[0].activityDurationInMinutes).to.equal(3);
    });

    it('Check if two activities with the same type are stored separately for scatterplot analysis', function(){
        var activityStoreModel = new ActivityStoreModel();
        var  dic;
        activityStoreModel.addActivityDataPoint(new ActivityData("A",dic,1));
        activityStoreModel.addActivityDataPoint(new ActivityData("A",dic,2));

        expect(activityStoreModel.activityList.length).to.equal(2);
        expect(activityStoreModel.activityList[0].activityDurationInMinutes).to.equal(1);
        expect(activityStoreModel.activityList[1].activityDurationInMinutes).to.equal(2);
    });

    it('Check if GraphModel has default graph types', function(){
        var graphModel = new GraphModel();
        expect(graphModel.getAvailableGraphNames() != null).to.be.true;
    });

    it('Check if GraphModel updates current graph type as user selects', function(){
        var graphModel = new GraphModel();
        graphModel.selectGraph("NewType");
        expect(graphModel.getNameOfCurrentlySelectedGraph() == "NewType").to.be.true;
    });

    it('Check remove activity',function(){
        var am = new ActivityStoreModel();
        var dic;
        var test = new ActivityData("a",dic,1);
        var test2 = new ActivityData("a",dic,2)
        am.addActivityDataPoint(test);
        am.removeActivityDataPoint(test2);
        expect(am.activityList.length == 1).to.be.true;
        expect(am.activityForDisplay.length == 1).to.be.true;

        am.removeActivityDataPoint(test);
        expect(am.activityList.length == 0).to.be.true;
        expect(am.activityForDisplay.length == 0).to.be.true;
    });
});
